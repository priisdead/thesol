...
`;
    ...
    class WebGLManager {
      ...
      loop() {
        ...
        // Frame-rate throttle: keep the fluid sim off the critical path so
        // it can't starve scroll/paint on the main thread.
        const now = performance.now();
    ...
    // Pause the WebGL loop while the user is actively scrolling. Because this
    // effect is a fixed full-viewport background, the IntersectionObserver
    // never marks it off-screen, so without this it renders every frame and
    // competes with scroll repaint — the source of the scroll lag. We freeze
    // it during the scroll and resume ~200ms after the last scroll event.
